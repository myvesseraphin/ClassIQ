...
import { Bell, ChevronLeft, X, Trash, CheckCircle } from "lucide-react";
...
const TeacherNotifications = () => {
  ...
  const markRead = async (id) => {
    try {
      const { data } = await api.patch(`/teacher/notifications/${id}/read`);
      setItems((s) =>
  ...
  const clearAll = async () => {
    try {
      await api.delete("/teacher/notifications");
      setItems([]);
    } catch (err) {
  ...
  if (isLoading) {
    return <TeacherPageSkeleton variant="notifications" />;
  }
          ...
          <div className="max-w-7xl mx-auto space-y-10">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                <button
                  onClick={() => navigate(-1)}
                  className="p-2 rounded-xl bg-white border border-slate-100 text-slate-500 hover:text-[#2D70FD] hover:border-blue-100"
                  aria-label="Go back"
                >
                  <ChevronLeft size={18} />
                </button>
                    ...
                    <div className="flex items-center gap-3">
                      {!n.read && (
                        <button
                          ...
                          className="px-3 py-2 bg-[#2D70FD] text-white rounded-2xl font-black text-xs"
                        >
                          Mark read
                        </button>
                      )}
