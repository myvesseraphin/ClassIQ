...
const TeacherSidebar = () => {
  ...
  useEffect(() => {
    const handleProfileUpdate = (event) => {
      ...
      if (!detail.avatarUrl && !detail.name && !detail.school) return;
      setProfileLoading(false);
      setTeacher((prev) => ({
        ...prev,
        avatarUrl: detail.avatarUrl ?? prev.avatarUrl,
        name: detail.name || prev.name,
        school: detail.school || prev.school,
      }));
    };
    window.addEventListener(
      "classiq:teacher-profile-updated",
      handleProfileUpdate,
    );
    return () => {
  ...
  return (
    <div className="h-screen w-[300px] bg-white flex flex-col border-r border-slate-200 shadow-sm">
        ...
        {menuItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) => `
              ...
              ${
                isActive
                  ? "bg-blue-50 text-[#2D70FD] shadow-sm font-bold"
      ...
      <div className="px-4 pb-10 space-y-4">
        <NavLink to="/teacher/profile" className="group block">
